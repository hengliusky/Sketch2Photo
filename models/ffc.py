...
class FFCUnit(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=7):
        super().__init__()
        self.reflect = nn.ReflectionPad2d(kernel_size // 2)
        self.ffc_bn_act = FFC_BN_ACT(in_channels=in_channels, out_channels=out_channels, kernel_size=kernel_size,
                                     padding=0, norm_layer=nn.BatchNorm2d,
                                     activation_layer=nn.ReLU, ratio_gin=0, ratio_gout=0.75, enable_lfu=False)
        self.res_block = FFCResnetBlock(out_channels, padding_type='reflect',
                                        activation_layer=nn.ReLU,
                                        norm_layer=nn.BatchNorm2d, ratio_gin=0.75, ratio_gout=0.75, enable_lfu=False)
        self.concat_layer = ConcatTupleLayer()

    def forward(self, x):
        return self.concat_layer(self.res_block(self.ffc_bn_act(self.reflect(x))))


if __name__ == '__main__':
    x = torch.randn(1, 3, 256, 256)
    ffc_unit = FFCUnit(in_channels=3, out_channels=64, kernel_size=7)
    out = ffc_unit(x)
